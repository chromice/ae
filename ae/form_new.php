...
class aeForm implements ArrayAccess, aeFormFieldFactory, aeFormGroupFactory
{
	use aeFormGroupAccess, aeFormGroupErrorContainer, aeFormGroupValueContainer;
	...
	// =====================================
	// = aeFormFieldFactory implementation =
	// =====================================
	...
	// =====================================
	// = aeFormGroupFactory implementation =
	// =====================================
	...
	public static function attributes($attributes)
	{
		...
		foreach ($attributes as $name => $value)
		{
			...
			if ($name === $value && in_array($name, array(
				'multiple', 'checked', 'selected', 'novalidate'
			)) || $value === true)
			{
...
class aeFormGroup implements ArrayAccess, aeFormFieldFactory
{
	use aeFormGroupAccess, aeFormGroupErrorContainer, aeFormGroupValueContainer;
	...
	public function single($name)
	{
		return $this->fields[$name] = new aeFormTextField($this->_field_name($name), null, false, $this->form, $this->values[$name], $this->errors[$name]);
	}
	
	public function multiple($name)
	{
		return $this->fields[$name] = new aeFormTextField($this->_field_name($name), null, true, $this->form, $this->values[$name], $this->errors[$name]);
	}
	...
	public function file($name, $destination)
	{
		...
		return $this->fields[$name] = new aeFormFileField($this->_field_name($name), null, false, $destination, $this->form, $this->values[$name], $this->errors[$name]);
	}
	...
	public function files($name, $destination)
	{
		...
		return $this->fields[$name] = new aeFormFileField($this->_field_name($name), null, true, $destination, $this->form, $this->values[$name], $this->errors[$name]);
	}
...
class aeFormSequence implements ArrayAccess, Iterator, Countable, aeFormFieldFactory
{
	protected $name;
	protected $form;
	protected $fields = array();
	...
	use aeFormSequenceAccess, aeFormSequenceIterator, aeFormSequenceCountable, aeFormGroupErrorContainer, aeFormGroupValueContainer;
...
abstract class aeFormField implements aeValidator
{
	...
	public function __construct($name, $index, $multiple, &$form, &$value)
	{
		$this->name = $name;
		$this->index = $index;
...
class aeFormTextField extends aeFormField implements aeTextValidator
{
	...
	public function __construct($name, $index, $multiple, &$form, &$value, &$error)
	{
		parent::__construct($name, $index, $multiple, $form, $value);
	...
	public function valid_value($message, $misc)
	{
		$this->validators[aeTextValidator::order_valid_value] = function ($value, $index = null) use ($message, $misc) {
			if (is_scalar($misc))
			{
				return is_array($value) && in_array($misc, $value)
					|| is_scalar($value) && $value == $misc ? null : $message;
			}
			elseif (is_array($misc))
			{
				return is_array($value) && count(array_intersect($value, $misc)) > 0
					|| is_scalar($value) && in_array($value, $misc) ? null : $message;
			}
...
class aeFormFileField extends aeFormField implements aeFileValidator
{
	...
	public function __construct($name, $index, $multiple, $destination, &$form, &$value, &$errors)
	{
		$this->errors =& $errors;
	...
	protected function _normalize_value()
	{
		...
		else
		{
			foreach ($this->value as $index => $value)
			{
				...
				if (!is_a($value, 'aeFile')/* || !$this->value[$index]->exists()*/)
				{
	...
	{
		return $this->name . ($this->multiple ? '[' . $offset . ']' : '');
	}
